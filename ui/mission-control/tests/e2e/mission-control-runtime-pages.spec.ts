...
test("runtime smoke covers terminal, live ops, connectors and connections", async ({ page }) => {
  ...
  await page.route("**/api/mt5/accounts", async (route, request) => {
    if (request.method() === "GET") {
      await route.fulfill({ json: [{ account_id: "mt5-demo-01", broker: "metaquotes", server: "MetaQuotes-Demo", login: "EXAMPLE_ACCOUNT_ID", mode: "paper", metadata: { broker_session: { snapshot_url: "http://mt5-bridge:18086/state.json", truth_source: "mt5-external-session-smoke" }, broker_runtime_session: { terminal: "mt5-main-live", connected: true } } }] });
      return;
    }
