  ...
  await page.route("**/api/system/live-ops", async (route) => {
    await route.fulfill({
      json: {
        ...
        watchdog_state: { status: "SAFE", health_score: 83, triggers: [] },
...
test.describe("dashboard route walkthrough smoke", () => {
  test.setTimeout(180_000);

  test("dashboard remains the canonical internal route and walkthrough starts on it", async ({ page }) => {
    await loginIfRequired(page, "/dashboard", "dashboard walkthrough smoke");

    await expect(page).toHaveURL(/\/dashboard(?:\?.*)?$/, { timeout: 30_000 });
    await expect(page.locator("#txt-global-nav[data-hydrated='1']")).toBeVisible({ timeout: 30_000 });
    await expect(page.locator("#global-guide-dashboard-hero")).toBeVisible({ timeout: 30_000 });
    ...
    await expect(dialog.getByText("Commence par le Dashboard", { exact: true })).toBeVisible();
    await expect(dialog.getByText("etape 1/8", { exact: false })).toBeVisible();
    await expect(dialog.getByText("page Dashboard", { exact: false })).toBeVisible();
  });

  test("hard-navigation shells route dashboard links to /dashboard", async ({ page }) => {
    await stubLiveOpsShell(page);
    await loginIfRequired(page, "/live-ops", "dashboard nav smoke");

    await expect(page.getByTestId("mission-control-live-ops-page")).toBeVisible({ timeout: 45_000 });
    await expect(page.locator("#txt-global-nav[data-hydrated='1']")).toBeVisible({ timeout: 30_000 });

    await page.locator("#txt-global-nav-link-dashboard").click();
