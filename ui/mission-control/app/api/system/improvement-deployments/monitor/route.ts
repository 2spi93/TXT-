...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const path = `/v1/system/improvement-deployments/monitor${query ? `?${query}` : ""}`;
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const { response, payload, network } = await cpFetchJsonSafe("/v1/system/improvement-deployments/monitor", {
    method: "POST",
    headers: {
