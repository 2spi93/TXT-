...
- The active Kairos default route now resolves to provider `mt5`, account `541283177`, `live_enabled=true`, `preferred_venue=mt5`.
...
- Routing is now correct: Kairos resolves to MT5 / `541283177` / venue `mt5`.
...
- Minimum live requirement: configure a real `broker_session.execution_url` for account `541283177`; otherwise the bridge returns `mt5_live_execution_unconfigured`.
...
curl -k -sS \
   -H "Authorization: Bearer $TOKEN" \
   -H 'content-type: application/json' \
   -X PATCH https://api.txt.gtixt.com/v1/mt5/accounts/541283177/broker-session \
